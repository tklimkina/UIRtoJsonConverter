...
/*----------------------------------------------------------------------------*/
/*    Copyright (c) National Instruments 1987-2006.  All Rights Reserved.     */
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Title:       toolbox.c                                                     */
...
#if HAVE_CVI_RTE && defined(_NI_mswin32_) && _NI_mswin32_
#include <cviauto.h>
#endif
#include <userint.h>
#include <libsupp.h>
...
/* Toolbox null check that sets error to the Toolbox out of memory code (-5071) instead of the User Interface Library one (-12) */
#ifndef tbNullChk
#define tbNullChk(fCall) if ((fCall) == 0) \
{error = ToolErr_OutOfMemory; goto Error;} else
#endif
...
#if (defined(_NI_mswin16_) && _NI_mswin16_) || (defined(_NI_mswin32_) && _NI_mswin32_)
...
{
#if !HAVE_CVI_RTE && !(defined(_NI_linux_) && _NI_linux_)
    int    cp = GetACP();
    CPINFO cpInfo;
...
static void CVIFUNC InitializeMultithreadingIfNecessary(void)
{
    ...
    if (!sMultithreadingInitDone)
        {
#if defined(_NI_linux_) && _NI_linux_
        if (sMultithreadingInitNow == 1)
            prev = sMultithreadingInitNow;
        else
            {
            pthread_mutex_lock (&mutex);
            prev = sMultithreadingInitNow;
            sMultithreadingInitNow = 1;
            pthread_mutex_unlock (&mutex);
            }
#else
...
#endif /* !(defined(_NI_linux_) && _NI_linux_) */
...
#define CNV_BROWSER_GAP         10
#define CNV_BROWSER_WIDTH       400
#define CNV_BROWSER_HEIGHT      300
#define CNV_BROWSER_DUMMY_TYPE  -1

typedef struct CNVBrowseInfo
{
    char            *name;
    int             leaf;
...
typedef struct CNVBrowserData
{
    CNVBrowser  browser;
    int         panel, tree, string, okBtn, cancelBtn, showDataTypes;
    char        *path;
} CNVBrowserData;

typedef int (CVIFUNC *  CNVCreateBrowser_t) (CNVBrowser * browser);
typedef int (CVIFUNC *  CNVBrowse_t) (CNVBrowser browser, const char * location);
typedef int (CVIFUNC *  CNVBrowseNextItem_t) (CNVBrowser browser, char ** item, int * leaf,
                            CNVBrowseType * browseType, CNVData * typeData);
typedef int (CVIFUNC *  CNVDisposeBrowser_t) (CNVBrowser browser);
typedef int (CVIFUNC *  CNVRegisterMachine_t) (const char * machine);
typedef int (CVIFUNC *  CNVUnregisterMachine_t) (const char * machine);
typedef void (CVIFUNC * CNVFreeMemory_t) (void * pointer);
typedef int (CVIFUNC *  CNVGetDataType_t) (CNVData data, CNVDataType * type, 
                            unsigned long * numDimensions);
typedef int (CVIFUNC * CNVDisposeData_t) (CNVData data);

static CNVCreateBrowser_t       CNVToolboxCreateBrowser;
static CNVBrowse_t              CNVToolboxBrowse;
static CNVBrowseNextItem_t      CNVToolboxBrowseNextItem;
static CNVDisposeBrowser_t      CNVToolboxDisposeBrowser;
static CNVRegisterMachine_t     CNVToolboxRegisterMachine;
static CNVUnregisterMachine_t   CNVToolboxUnregisterMachine;
static CNVFreeMemory_t          CNVToolboxFreeMemory;
static CNVGetDataType_t         CNVToolboxGetDataType;
static CNVDisposeData_t         CNVToolboxDisposeData;

static HINSTANCE    cnvDLLHandle = 0;
static int          cnvLibInitComplete = FALSE;

static int CVICALLBACK CNVBrowserPanelCallback(int panel, int event, void *callbackData,
        int eventData1, int eventData2);
static int CVICALLBACK CNVBrowserTreeCallback(int panel, int control, int event,
        void *callbackData, int eventData1, int eventData2);
static int CVICALLBACK CNVBrowserOKCallback(int panel, int control, int event, 
        void *callbackData, int eventData1, int eventData2);
static int CVICALLBACK CNVBrowserCancelCallback(int panel, int control, int event, 
        void *callbackData, int eventData1, int eventData2);
static void CVICALLBACK CNVBrowserAddMachineCallback(int panel, int control, int menuItem, 
        void *callbackData);
static void CVICALLBACK CNVBrowserRemoveMachineCallback(int panel, int control, 
        int menuItem, void *callbackData);
static int GetPathFromNode(CNVBrowserData * data, int index, char ** path);
static int Browse(CNVBrowserData * data);
static int BrowseChildren(CNVBrowserData * data, int index);

static int GetNetworkVariableFuncsIfNeeded(void)
{
    HANDLE  mutexHandle;
    DWORD   processId;
    char    mutexName[MAX_PATH+1];
    HMODULE runtimeDLL;
    char    path[1024], *lastSlash;

    if (cnvLibInitComplete) 
        return 0;

    processId = GetCurrentProcessId();
    sprintf (mutexName, "National Instruments LabWindows/CVI Toolbox cvinetv %d", processId);
    mutexHandle = CreateMutex(NULL, FALSE, mutexName);
    if (mutexHandle == NULL)
        return CNVCouldNotCreateMutexError;
    ...
    runtimeDLL = GetModuleHandle("cvirte.dll");
    if (runtimeDLL == NULL)
        runtimeDLL = GetModuleHandle("instrsup.dll");

    if (runtimeDLL != NULL)
    {
        /* Give preference to private RTE */
        if (GetModuleFileName(runtimeDLL, path, sizeof(path)) > 0)
        {
            lastSlash = (char *) _mbsrchr((unsigned char *)path, '\\');
            if (lastSlash != NULL)
            {
                *(lastSlash + 1) = '\0';
                strcat(path, "ninetv.dll");
                cnvDLLHandle = LoadLibrary(path);
            }
        }
    }

    if (cnvDLLHandle == NULL)
    {
        unsigned int size;
    
        if (RegReadString(REGKEY_HKLM, "SOFTWARE\\National Instruments\\Common\\Installer",
            "NISHAREDDIR", (unsigned char *)path, sizeof(path), &size) >= 0)
        {
            size = strlen(path);
            lastSlash = (char *) _mbsrchr((unsigned char *)path, '\\');
            if (lastSlash == NULL || ((DWORD) (lastSlash - path)) < (size - 1))
                strcat(path, "\\");
            strcat(path, "Network Variable\\"); 
            strcat(path, "ninetv.dll");
            cnvDLLHandle = LoadLibrary(path);
        }
    }
    ...
    if (cnvDLLHandle == NULL)
        cnvDLLHandle = LoadLibrary("ninetv.dll");

    if (cnvDLLHandle == NULL)
    {
        ReleaseMutex(mutexHandle);
        CloseHandle(mutexHandle);
        return CNVLibCouldNotBeLoadedError;
    }

    if (!(CNVToolboxCreateBrowser = (void*) GetProcAddress(cnvDLLHandle, "CNVCreateBrowser")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxBrowse = (void*) GetProcAddress(cnvDLLHandle, "CNVBrowse")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxBrowseNextItem = (void*) GetProcAddress(cnvDLLHandle, "CNVBrowseNextItem")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxDisposeBrowser = (void*) GetProcAddress(cnvDLLHandle, "CNVDisposeBrowser")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxRegisterMachine = (void*) GetProcAddress(cnvDLLHandle, "CNVRegisterMachine")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxUnregisterMachine = (void*) GetProcAddress(cnvDLLHandle, "CNVUnregisterMachine")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxFreeMemory = (void*) GetProcAddress(cnvDLLHandle, "CNVFreeMemory")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxGetDataType = (void*) GetProcAddress(cnvDLLHandle, "CNVGetDataType")))
        goto FunctionNotFoundError;

    if (!(CNVToolboxDisposeData = (void*) GetProcAddress(cnvDLLHandle, "CNVDisposeData")))
        goto FunctionNotFoundError;

    cnvLibInitComplete = TRUE;
    ReleaseMutex(mutexHandle);
    CloseHandle(mutexHandle);
    return 0;

FunctionNotFoundError:
    FreeLibrary(cnvDLLHandle);
    cnvDLLHandle = NULL;
    ReleaseMutex(mutexHandle);
    CloseHandle(mutexHandle);
    return CNVInvalidLibVersionError;
}

static int CVIFUNC GetCVIBitmapFromHIcon(HICON icon, int* bitmap)
{
    int             error = 0, i, rowSize;
    DWORD_PTR       win_error;
    ...
    errChk(NewBitmap(rowSize,                           // bytes per row
                     ...
                     (unsigned char*)bitmapData,        // the bitmap iteself
...
{
    int             error = 0, btnWidth, btnHeight, stringHeight, removePopup = 0, 
                    machineBmp = 0, folderBmp = 0, itemBmp = 0, labelWidth;
    HANDLE          machineImage = 0, folderImage = 0, itemImage = 0;
    CNVBrowserData  data;
    
    if (!selectedItem)
        return UIENullPointerPassed;
    
    errChk(GetNetworkVariableFuncsIfNeeded());

    memset(&data, 0, sizeof(data));
    data.showDataTypes = 1;
    data.path = selectedItem;

    errChk(CNVToolboxCreateBrowser(&data.browser));
    machineImage = LoadImage(cnvDLLHandle, MAKEINTRESOURCE(1), IMAGE_ICON, 0, 0, 0);
    if (machineImage == NULL)
        errChk(UIEResourceNotFound);
    folderImage = LoadImage(cnvDLLHandle, MAKEINTRESOURCE(2), IMAGE_ICON, 0, 0, 0);
    if (folderImage == NULL)
        errChk(UIEResourceNotFound);
    itemImage = LoadImage(cnvDLLHandle, MAKEINTRESOURCE(3), IMAGE_ICON, 0, 0, 0);
    if (itemImage == NULL)
        errChk(UIEResourceNotFound);
    errChk(GetCVIBitmapFromHIcon(machineImage, &machineBmp));
    errChk(GetCVIBitmapFromHIcon(folderImage, &folderBmp));
    errChk(GetCVIBitmapFromHIcon(itemImage, &itemBmp));

    errChk(NewPanel(0, "Browse Network Variables", VAL_AUTO_CENTER,
        VAL_AUTO_CENTER, CNV_BROWSER_HEIGHT, CNV_BROWSER_WIDTH));
    data.panel = error;
    errChk(NewCtrl(data.panel, CTRL_TREE_LS, "", CNV_BROWSER_GAP, CNV_BROWSER_GAP));
    data.tree = error;
    errChk(NewCtrlMenuItem(data.panel, data.tree, "__Add Machine...", -4, 
        CNVBrowserAddMachineCallback, &data));
    errChk(NewCtrlMenuItem(data.panel, data.tree, "__Remove Machine", -4, 
        CNVBrowserRemoveMachineCallback, &data));
    errChk(NewCtrlMenuSeparator(data.panel, data.tree, -4));
    errChk(NewCtrl(data.panel, CTRL_STRING_LS, "Variable __Path: ", 0, CNV_BROWSER_GAP));
    data.string = error;
    errChk(NewCtrl(data.panel, CTRL_SQUARE_COMMAND_BUTTON_LS, "OK", 0, 0));
    data.okBtn = error;
    errChk(NewCtrl(data.panel, CTRL_SQUARE_COMMAND_BUTTON_LS, "Cancel", 0, 0));
    data.cancelBtn = error;
    errChk(InstallPanelCallback(data.panel, CNVBrowserPanelCallback, &data));
    errChk(InstallCtrlCallback(data.panel, data.tree, CNVBrowserTreeCallback, &data));
    errChk(InstallCtrlCallback(data.panel, data.okBtn, CNVBrowserOKCallback, &data));
    errChk(InstallCtrlCallback(data.panel, data.cancelBtn, CNVBrowserCancelCallback, &data));
    errChk(SetPanelAttribute(data.panel, ATTR_CONFORM_TO_SYSTEM, 1));
    errChk(SetPanelAttribute(data.panel, ATTR_CONFORM_TO_SYSTEM_THEME, 1));
    errChk(SetCtrlAttribute(data.panel, data.okBtn, ATTR_SHORTCUT_KEY, VAL_ENTER_VKEY));
    errChk(SetCtrlAttribute(data.panel, data.cancelBtn, ATTR_SHORTCUT_KEY, VAL_ESC_VKEY));
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_MAX_ENTRY_LENGTH,
        MAX_PATHNAME_LEN - 1));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_CTRL_MODE, VAL_HOT));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_DATA_TYPE, VAL_INTEGER));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_SELECTION_MODE, VAL_SELECTION_SINGLE));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_ENABLE_DRAG_DROP, 0));
    errChk(SetTreeItemAttribute(data.panel, data.tree, VAL_DFLT_FOR_NEW_OBJECTS, ATTR_NO_EDIT_LABEL, 1));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_SHOW_IMAGES, 1));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_DOUBLE_CLICK_TOGGLES_COLLAPSE, 1));
    errChk(AddTreeImage(data.panel, data.tree, machineBmp));
    errChk(AddTreeImage(data.panel, data.tree, folderBmp));
    errChk(AddTreeImage(data.panel, data.tree, itemBmp));

    errChk(GetCtrlAttribute(data.panel, data.string, ATTR_HEIGHT, &stringHeight));
    errChk(GetCtrlAttribute(data.panel, data.string, ATTR_LABEL_WIDTH, &labelWidth));
    errChk(GetCtrlAttribute(data.panel, data.cancelBtn, ATTR_WIDTH, &btnWidth));
    btnWidth += 2 * CNV_BROWSER_GAP;
    errChk(GetCtrlAttribute(data.panel, data.cancelBtn, ATTR_HEIGHT, &btnHeight));

    errChk(SetCtrlAttribute(data.panel, data.okBtn, ATTR_WIDTH, btnWidth));
    errChk(SetCtrlAttribute(data.panel, data.cancelBtn, ATTR_WIDTH, btnWidth));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_WIDTH,
        CNV_BROWSER_WIDTH - 2 * CNV_BROWSER_GAP));
    errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_HEIGHT,
        CNV_BROWSER_HEIGHT - 4 * CNV_BROWSER_GAP - stringHeight - btnHeight));
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_WIDTH,
        CNV_BROWSER_WIDTH - 3 * CNV_BROWSER_GAP - labelWidth));
    
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_TOP,
        CNV_BROWSER_HEIGHT - 2 * CNV_BROWSER_GAP - stringHeight - btnHeight));
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_LABEL_TOP,
        CNV_BROWSER_HEIGHT - 2 * CNV_BROWSER_GAP - stringHeight - btnHeight));
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_LEFT,
        2 * CNV_BROWSER_GAP + labelWidth));
    errChk(SetCtrlAttribute(data.panel, data.string, ATTR_LABEL_LEFT,
        CNV_BROWSER_GAP));
    errChk(SetCtrlAttribute(data.panel, data.okBtn, ATTR_TOP,
        CNV_BROWSER_HEIGHT - CNV_BROWSER_GAP - btnHeight));
    errChk(SetCtrlAttribute(data.panel, data.cancelBtn, ATTR_TOP,
        CNV_BROWSER_HEIGHT - CNV_BROWSER_GAP - btnHeight));
    errChk(SetCtrlAttribute(data.panel, data.okBtn, ATTR_LEFT,
        CNV_BROWSER_WIDTH / 2 - btnWidth - 2 * CNV_BROWSER_GAP));
    errChk(SetCtrlAttribute(data.panel, data.cancelBtn, ATTR_LEFT,
        CNV_BROWSER_WIDTH / 2 + 2 * CNV_BROWSER_GAP));
    
    if (data.showDataTypes)
    {
        errChk(SetTreeColumnAttribute(data.panel, data.tree, 0,
            ATTR_LABEL_TEXT, "Variables"));
        errChk(InsertTreeColumn(data.panel, data.tree, -1, "Type"));
        errChk(SetCtrlAttribute(data.panel, data.tree, ATTR_COLUMN_LABELS_VISIBLE, 1));
    }

    errChk(Browse(&data));

    errChk(InstallPopup(data.panel));
    removePopup = 1;
    errChk(RunUserInterface());

Error:
    if (machineBmp)
        DiscardBitmap(machineBmp);
    if (folderBmp)
        DiscardBitmap(folderBmp);
    if (itemBmp)
        DiscardBitmap(itemBmp);
    if (machineImage)
        DestroyIcon(machineImage);
    if (folderImage)
        DestroyIcon(folderImage);
    if (itemImage)
        DestroyIcon(itemImage);
    if (removePopup)
        RemovePopup(0);
    if (data.browser)
        CNVToolboxDisposeBrowser(data.browser);
    if (data.panel)
        DiscardPanel(data.panel);
    return error;
}
...
static int GetDisplayPathFromNode(CNVBrowserData * data, int index, char ** path)
{
    int             error = 0, parentIndex, nameLen;
    char            *p = 0, *name = 0;
    CNVBrowseType   browseType;

    errChk(GetTreeItemParent(data->panel, data->tree, index, &parentIndex));
    if (parentIndex == -1)
        nullChk(p = StrDup("\\"));
    else
        errChk(GetDisplayPathFromNode(data, parentIndex, &p));
    errChk(GetTreeItemAttribute(data->panel, data->tree, index, ATTR_CTRL_VAL, (int*)&browseType));
    if (browseType == CNVBrowseTypeImplicitItem)
        goto Done;
    
    errChk(GetTreeItemAttribute(data->panel, data->tree, index, ATTR_LABEL_TEXT_LENGTH, &nameLen));
    nullChk(name = malloc(nameLen + 1));
    errChk(GetTreeItemAttribute(data->panel, data->tree, index, ATTR_LABEL_TEXT, name));
    if (browseType == CNVBrowseTypeItemRange)
    {
        char * ptr;
        
        ptr = _mbsrchr((unsigned char *)name, '-');
        if (ptr != NULL)
        {
            char * tmp;
            
            *ptr = '\0';
            nullChk(tmp = StrDupWithoutSurrWhiteSpace(name));
            free(name);
            name = tmp;
            ...
            while (tmp != NULL)
            {
                ...
                if (tmp == NULL)
                    break;
                if (tmp != name)
                    ptr = _mbsdec(name, tmp);
                if (tmp == name || (ptr != NULL && *ptr == '\\'))
                {
                    errChk(AppendString(&name, "'", 1));
                    break;
                }
                ++tmp;
            }
    ...
    errChk(AppendString(&p, "\\", 1));
    errChk(AppendString(&p, name, -1));
...
static int BrowserExpandOneMachine(CNVBrowserData * data, int itemIndex)
{
    int     error = 0, numChildren, childIndex, browseType;
    
    errChk(GetTreeItemAttribute(data->panel, data->tree, itemIndex, ATTR_CTRL_VAL, &browseType));
    if (browseType != CNVBrowseTypeMachine)
        goto Error;
    errChk(GetTreeItemNumChildren(data->panel, data->tree, itemIndex, &numChildren));
    if (numChildren != 1)
        goto Error;
    errChk(GetTreeItem(data->panel, data->tree, VAL_CHILD, itemIndex, VAL_FIRST, 
        VAL_NEXT_PLUS_SELF, VAL_LEAF, &childIndex));
    if (childIndex < 0)
        goto Error;
    errChk(GetTreeItemAttribute(data->panel, data->tree, childIndex, ATTR_CTRL_VAL, &browseType));
    if (browseType != CNV_BROWSER_DUMMY_TYPE)
        goto Error;
    errChk(DeleteListItem(data->panel, data->tree, childIndex, 1));
    errChk(BrowseChildren(data, itemIndex));
...
static int CVICALLBACK CNVBrowserTreeCallback(int panel, int control, int event,
                            void *callbackData, int eventData1, int eventData2)
{
    int             error = 0, index, waitCursor = 0;
    char            *path = 0;
    CNVBrowserData  *data = (CNVBrowserData *) callbackData;
    ...
    switch (event)
    {
        case EVENT_VAL_CHANGED:
            ...
            {
                errChk(GetActiveTreeItem(data->panel, data->tree, &index));
                errChk(GetDisplayPathFromNode(data, index, &path));
                errChk(SetCtrlVal(data->panel, data->string, path));
            }
            break;
        case EVENT_EXPAND:
            ...
            if (eventData1 == VAL_ALL)
            {
                int item = 0;
        
                while (item >= 0)
                {
                    errChk(BrowserExpandOneMachine(data, item));
                    errChk(GetTreeItem(data->panel, data->tree, VAL_SIBLING, item, item, 
                        VAL_NEXT, 0, &item));
                }
            }
            else
            {
                errChk(BrowserExpandOneMachine(data, eventData2));
            }
            break;
        case EVENT_COMMIT:
            if (eventData1 == LEFT_DOUBLE_CLICK)
            {
                int browseType;
                errChk(GetActiveTreeItem(data->panel, data->tree, &index));
                errChk(GetTreeItemAttribute(data->panel, data->tree, index, ATTR_CTRL_VAL, &browseType));
                if (browseType == CNVBrowseTypeItem || browseType == CNVBrowseTypeItemRange
                        || browseType == CNVBrowseTypeImplicitItem)
                    CNVBrowserOKCallback(panel, control, EVENT_COMMIT, callbackData, 0, 0);
            }
            break;
    }
...
static void CVICALLBACK CNVBrowserAddMachineCallback(int panel, int control, int menuItem, 
                            void *callbackData)
{
    int             error = 0, item, waitCursor = 0;
    CNVBrowserData  *data = (CNVBrowserData *) callbackData;
    char            machine[256];

    errChk(PromptPopup("Add Machine", "Name of machine to add:", 
        machine, sizeof(machine) - 1));
    if (machine[0] != '\0')
    {
        if (CNVToolboxRegisterMachine(machine) < 0)
        {
            MessagePopup("Error", "Could not register machine");
            goto Error;
        }
        
        errChk(InsertTreeItem(data->panel, data->tree, VAL_SIBLING, 0, VAL_LAST, 
            machine, 0, 0, CNVBrowseTypeMachine));
        item = error;
        errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_IMAGE_INDEX, 0));
        errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_COLLAPSED_IMAGE_INDEX, 0));
        errChk(SetActiveTreeItem(data->panel, data->tree, item, VAL_REPLACE_SELECTION_WITH_ITEM));
        errChk(SetCtrlVal(data->panel, data->string, machine));
        errChk(SetWaitCursor(1));
        waitCursor = 1;
        BrowseChildren(data, item);
        errChk(SetCtrlAttribute(data->panel, data->tree, ATTR_FIRST_VISIBLE_ROW, item));
    }
...
static void CVICALLBACK CNVBrowserRemoveMachineCallback(int panel, int control, 
                            int menuItem, void *callbackData)
{
    int             error = 0, item, parent, nameLen;
    CNVBrowserData  *data = (CNVBrowserData *) callbackData;
    char            *name = NULL;

    errChk(GetActiveTreeItem(data->panel, data->tree, &item));
    while (1)
    {
        errChk(GetTreeItemParent(data->panel, data->tree, item, &parent));
        if (parent == -1)
            break;
        item = parent;
    }
    errChk(GetTreeItemAttribute(data->panel, data->tree, item, ATTR_LABEL_TEXT_LENGTH, &nameLen));
    nullChk(name = malloc(nameLen + 1));
    errChk(GetTreeItemAttribute(data->panel, data->tree, item, ATTR_LABEL_TEXT, name));
    errChk(DeleteListItem(data->panel, data->tree, item, 1));
        
    if (CNVToolboxUnregisterMachine(name) < 0)
    {
        MessagePopup("Error", "Could not unregister machine");
        goto Error;
    }
                            ...
                            void *callbackData, int eventData1, int eventData2)
{
    ...
    {
        case EVENT_COMMIT:
            errChk(GetCtrlVal(data->panel, data->string, data->path));
...
static int CVICALLBACK DisposeBrowseInfo(int index, void *itemPtr, void *data)
{
    CNVBrowseInfo * item = (CNVBrowseInfo *) itemPtr;

    if (item && item->name)
    {
        CNVToolboxFreeMemory(item->name);
        item->name = 0;
    }
...
static void DisposeBrowseInfoList(ListType list)
{
    if (list)
    {
        ListApplyToEach(list, 1, DisposeBrowseInfo, 0);
        ListDispose(list);
    }
}

static int GetBrowseInfo(CNVBrowserData * data, const char * path, ListType * list)
{
    int             error = 0;
    CNVBrowseInfo   info;
    ListType        lt = 0;
    ...
    nullChk(lt = ListCreate(sizeof(CNVBrowseInfo)));
    errChk(CNVToolboxBrowse(data->browser, path));
    while (1)
    {
        memset(&info, 0, sizeof(info));
        errChk(CNVToolboxBrowseNextItem(data->browser, &info.name, &info.leaf, 
            &info.browseType, &typeData));
        if (error == 0)
            break;
        // NOTE - Not error checking to make sure we process unsupported data
        CNVToolboxGetDataType(typeData, &info.type, &info.nDims);
        CNVToolboxDisposeData(typeData);
        typeData = 0;
        nullChk(ListInsertItem(lt, &info, END_OF_LIST));
    }
    *list = lt;
    lt = 0;
...
static int ProcessBrowseInfoList(CNVBrowserData * data, int relative, int relation, 
                ListType list, int browseChildren)
{
    int             error = 0, i, j, nItems, item, imageIndex = 1;
    CNVBrowseInfo   *info;
    char            *typeString = NULL;

    nItems = ListNumItems(list);
    for (i = 1; i <= nItems; ++i)
    {
        nullChk(info = ListGetPtrToItem(list, i));
        errChk(InsertTreeItem(data->panel, data->tree, relation, relative,
            VAL_LAST, info->name, 0, 0, info->browseType));
        item = error;
        switch (info->browseType)
        {
        ...
        errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_IMAGE_INDEX, imageIndex));
        errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_COLLAPSED_IMAGE_INDEX, imageIndex));
    
        if (!browseChildren)
        {
            errChk(InsertTreeItem(data->panel, data->tree, VAL_CHILD, item,
                VAL_LAST, "", 0, 0, CNV_BROWSER_DUMMY_TYPE));
        }
        else
        {
            if (data->showDataTypes && info->type > 0)
            {
                if (info->nDims == 0)
                {
                    errChk(SetTreeCellAttribute(data->panel, data->tree, item, 1,
                        ATTR_LABEL_TEXT, TypeNames[info->type]));
                }
                else
                {
                    nullChk(typeString = malloc(strlen(TypeNames[info->type]) + 2 * info->nDims + 1));
                    strcpy(typeString, TypeNames[info->type]);
                    for (j = 0; j < (int)info->nDims; ++j)
                        strcat(typeString, "[]");
                
                    errChk(SetTreeCellAttribute(data->panel, data->tree, item, 1,
                        ATTR_LABEL_TEXT, typeString));
                    free(typeString);
                    typeString = NULL;
                }
            }
            ...
            if (!info->leaf)
            {
                errChk(BrowseChildren(data, item));
                errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_COLLAPSED, 1));
            }
        }
    }
...
static int GetPathFromNode(CNVBrowserData * data, int index, char ** path)
{
    ...
    errChk(GetTreeItemPathLength(data->panel, data->tree, index, "\\", &pathLen));
    nullChk(p = malloc(pathLen + 3));
    p[0] = p[1] = '\\';
    errChk(GetTreeItemPath(data->panel, data->tree, index, "\\", p + 2));
    *path = p;
    p = 0;
...
static int BrowseChildren(CNVBrowserData * data, int index)
{
    ...
    errChk(GetPathFromNode(data, index, &path));
    errChk(GetBrowseInfo(data, path, &list));
    errChk(ProcessBrowseInfoList(data, index, VAL_CHILD, list, 1));

Error:
    free(path);
    DisposeBrowseInfoList(list);
    return error;
}

static int Browse(CNVBrowserData * data)
{
    int         error = 0, treeWidth, colWidth, scrollBarSize, frameThickness, item;
    ListType    list = 0;

    errChk(InsertTreeItem(data->panel, data->tree, VAL_SIBLING, 0, VAL_NEXT,
        "localhost", 0, 0, CNVBrowseTypeMachine));
    item = error;
    errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_IMAGE_INDEX, 0));
    errChk(SetTreeItemAttribute(data->panel, data->tree, item, ATTR_COLLAPSED_IMAGE_INDEX, 0));
    errChk(InsertTreeItem(data->panel, data->tree, VAL_CHILD, item,
    ...
    errChk(GetBrowseInfo(data, 0, &list));
    errChk(ProcessBrowseInfoList(data, item, VAL_SIBLING, list, 0));

    errChk(GetCtrlAttribute(data->panel, data->tree, ATTR_WIDTH, &treeWidth));
    errChk(GetCtrlAttribute(data->panel, data->tree, ATTR_SCROLL_BAR_SIZE, &scrollBarSize));
    errChk(GetCtrlAttribute(data->panel, data->tree, ATTR_FRAME_THICKNESS, &frameThickness));
    if (data->showDataTypes)
    {
    ...
    {
        errChk(SetTreeColumnAttribute(data->panel, data->tree, 0, ATTR_COLUMN_WIDTH,
            treeWidth - scrollBarSize - 2 * frameThickness));
    }

    errChk(SetTreeItemAttribute(data->panel, data->tree, VAL_ALL_OBJECTS, ATTR_COLLAPSED, 1));
    errChk(SetActiveTreeItem(data->panel, data->tree, 0, VAL_REPLACE_SELECTION_WITH_ITEM));
    CNVBrowserTreeCallback(data->panel, data->tree, EVENT_VAL_CHANGED, data, ACTIVE_ITEM_CHANGE, 0);
...
{
    const unsigned char *firstNonWhiteChar = NULL, *lastNonWhiteChar = NULL;
    char *copy = NULL;
    int   len;
    ...
    if (lastNonWhiteChar > firstNonWhiteChar)
        ...
        CmbStrDec(firstNonWhiteChar, lastNonWhiteChar);
...
void CVIFUNC RemoveSurroundingWhiteSpace(char *string)
{
    ...
    if (string)
        ...
        while (end < string+lengthWithoutLeadingWS)
        {
            if (!isspace(*end))
                lastNonWS  = end;
            CmbStrInc((unsigned char *)end);
        }

        if (end != string)
            CmbStrInc((unsigned char *)lastNonWS);
        *lastNonWS = 0;
        }
}
...
int CVIFUNC StrICmpWithoutSurrWhiteSpace (const char *s1, const char *s2)
{
    ...
    if (s1 == NULL)
        ...
        if (s2 == NULL)
            ...
            for (;; CmbStrInc((unsigned char *)s1),CmbStrInc((unsigned char *)s2))
                {
                ch1 = CmbGetC(s1);
                if (CmbIsSingleC(ch1))
                    ch1 = toupper(ch1);
...
{
#if defined(_NI_mswin_) && _NI_mswin_ /* _NI_mswin_ is defined automatically when compiling with the Windows version of CVI. */
...
/* 
    The following is the lookup3 hash function implementation by Bob Jenkins. 
    The original code can be found at http://www.burtleburtle.net/bob/c/lookup3.c
    and his personal page is http://www.burtleburtle.net/bob/index.html.
    Thanks, Bob.
*/
...
uint32_t CVICALLBACK Lookup3_hashlittle(const void *key, size_t length) /* moved the initval parameter into the function */
{
  static uint32_t initval = 0xDEADF00D;
...
#if defined(VALGRIND) || defined(_CVI_DEBUG_) 
    const uint8_t  *k8;
#endif
...
typedef struct _KeyValuePairNode    /* The hash table uses chaining to resolve collisions, so buckets store linked lists */
    {
    void                        *key;
    void                        *value;
    struct _KeyValuePairNode    *next;
    } KeyValuePairNode, *KeyValuePairNodePtr;

typedef struct _HashBucket
    {
    unsigned long       chainLength;
    KeyValuePairNodePtr first;
    } HashBucket; 

typedef int (CVIANSI * KeyCmpFunc)(const void *, const void *, size_t); 
...
typedef struct _HashTable
    ...
    size_t              keySize;
    size_t              valueSize;
    HashBytesFunc       hashFunc;
    KeyCmpFunc          keyCmpFunc;
    unsigned long       initialNumBuckets;
    double              growLoad;
    double              shrinkLoad;

    HashBucket          *buckets;
    unsigned long       size;
    unsigned long       numOccupiedBuckets;
    unsigned long       numBuckets;         /* HashTableCreate ensures this is never 0 */
    
    unsigned __int64    *iterators;
    unsigned long       iteratorsCapacity;
    } HashTable;

#define HashTableSignature (0xDEADBABE)
#define ITER_RESERVED_BIT_MASK ((unsigned __int64)0x8000000000000000)
#define ITER_VALID_BIT_MASK    ((unsigned __int64)0x4000000000000000)
#define ITER_AT_END_BIT_MASK   ((unsigned __int64)0x2000000000000000)

/* reasonable hash functions begin to see an increase in collisions and degradation of table performance at around 80% load */
static const double         kFewCollisionsThresholdLoad = 0.80; 
static const unsigned long  kInitIteratorCapacity       = 2;
static const double         kMinGrowLoad                = 0.50;
static const double         kMaxShrinkLoad              = 0.50;
static const unsigned int   kMaxSupportedIterators      = (unsigned int)-1;
static const unsigned long  kMaxChainLength             = 1 << 28; // limited by iterator representation
static const unsigned long  kMaxNumBuckets              = 1 << 31; // limited by iterator representation 

#define VALIDATE_TABLE(T)           do { if (!(T) || (T)->signature != HashTableSignature) errChk (ToolErr_InvalidHashTable); } while (0)
#define VALIDATE_KEY(K)             do { if (!(K)) errChk (ToolErr_InvalidKeyArgument); } while (0)
#define VALIDATE_VALUE(V)           do { if (!(V)) errChk (ToolErr_InvalidValueArgument); } while (0)

#define SET_ITERATOR(T,I,B,N)       ((T)->iterators[(unsigned)I] = ((B) | (((unsigned __int64)(N)) << 32) | ITER_RESERVED_BIT_MASK | ITER_VALID_BIT_MASK) & ~ITER_AT_END_BIT_MASK)
#define ITERATOR_IS_RESERVED(T,I)   ((T)->iterators[(unsigned)I] & ITER_RESERVED_BIT_MASK)
#define ITERATOR_IS_VALID(T,I)      ((T)->iterators[(unsigned)I] & ITER_VALID_BIT_MASK)
#define ITERATOR_IS_AT_END(T,I)     ((T)->iterators[(unsigned)I] & ITER_AT_END_BIT_MASK)
#define RESERVE_ITERATOR(T,I)       ((T)->iterators[(unsigned)I] |= ITER_RESERVED_BIT_MASK)
#define FREE_ITERATOR(T,I)          ((T)->iterators[(unsigned)I] &= ~ITER_RESERVED_BIT_MASK)
#define INVALIDATE_ITERATOR(T,I)    ((T)->iterators[(unsigned)I] &= ~ITER_VALID_BIT_MASK)
#define SET_ITERATOR_AT_END(T,I)    ((T)->iterators[(unsigned)I] |= ITER_AT_END_BIT_MASK)
#define READ_ITERATOR(T,I,B,N)      do { unsigned __int64 _u64Iter = (T)->iterators[(unsigned)I]; (N) = (unsigned long)((_u64Iter & ~(ITER_RESERVED_BIT_MASK | ITER_VALID_BIT_MASK)) >> 32); (B) = (unsigned long)_u64Iter; } while (0)
#define VALIDATE_ITERATOR_RESERVED(T,I) \
    if ((unsigned)(I) == 0\
        || (unsigned)(I) >= (T)->iteratorsCapacity\
        || !ITERATOR_IS_RESERVED(T, I))\
        errChk (ToolErr_UnallocatedIterator);\
    else

#define VALIDATE_ITERATOR_HAS_GOOD_ITEM(T,I) \
do { \
    VALIDATE_ITERATOR_RESERVED (T, I);\
    if (!ITERATOR_IS_VALID(T,I)) {\
        errChk (ToolErr_InvalidIterator);\
    } else if (ITERATOR_IS_AT_END(T,I)) {\
        errChk (ToolErr_IteratorAtEnd);\
    }\
} while (0)

#define HASH_KEY(T,K,s,h) \
do {\
    s = ((T)->keyType == FIXED_SIZE_KEY) ? (T)->keySize : strlen(K) + 1;\
    h = (T)->hashFunc(K, s) & ((T)->numBuckets - 1);\
} while (0)

#define INVALIDATE_ALL_ITERATORS(T) \
do {\
    if ((T)->iterators)\
    {\
        unsigned long i, n = (T)->iteratorsCapacity;\
        for (i = 0; i < n; ++i)\
            INVALIDATE_ITERATOR (T,i);\
    }\
} while (0)

static void HashTableDeallocate(HashTableType table);
static int HashTableGrow(HashTableType table);
static int HashTableShrink(HashTableType table);
static int HashTableResize(HashTableType table, unsigned long newNumBuckets);
static int HashTableInsertNodeInTable(HashTableType table, KeyValuePairNodePtr *pNode, KeyHash hash);
static int HashTableGetItemInternal(HashTableType table, const void *key, KeyValuePairNodePtr *node, int remove);
static int CVIANSI StrcmpWrapper(const void *str1, const void *str2, size_t unused);
...
{
    int                 error                   = 0;
    const unsigned long minimumCapacity         = 32;
    unsigned long       goodPerformanceCapacity,
                        pow2;
    HashTableType       ht                      = NULL;

    if (table)
        *table = NULL; // ensure that no matter what error is reported, the output param is nulled out

    if (keyType != FIXED_SIZE_KEY && keyType != C_STRING_KEY)
    {
        errChk (ToolErr_InvalidKeyType);
    }
    if (keyType == FIXED_SIZE_KEY && keySize == 0)
    {
        errChk (ToolErr_InvalidKeySize);
    }
    if (valueSize == 0)
    {
        errChk (ToolErr_InvalidValueSize);
    }
    if (!table)
    {
        errChk (ToolErr_NullArgument);
    }

    goodPerformanceCapacity = (unsigned long) ceil((double)initialSize / kFewCollisionsThresholdLoad);
    if (goodPerformanceCapacity < minimumCapacity)
        goodPerformanceCapacity = minimumCapacity;
    for (pow2 = 2; pow2 < goodPerformanceCapacity; pow2 <<= 1);
    
    if (pow2 > kMaxNumBuckets)
        errChk (ToolErr_ExceedsMaximumTableSize);

    tbNullChk (ht = calloc(sizeof(HashTable), 1));

    ht->signature               = HashTableSignature;
    ht->keyType                 = keyType;
    ht->keySize                 = keySize;
    ht->valueSize               = valueSize;
    ht->hashFunc                = Lookup3_hashlittle;
    ht->keyCmpFunc              = (keyType == C_STRING_KEY) ? StrcmpWrapper : memcmp;
    ht->growLoad                = kFewCollisionsThresholdLoad;
    ht->initialNumBuckets       = pow2;
    ht->shrinkLoad              = VAL_DO_NOT_SHRINK;

    //ht->size                  = 0;
    //ht->numOccupiedBuckets    = 0;
    ht->numBuckets              = ht->initialNumBuckets;
    tbNullChk (ht->buckets      = calloc(sizeof(HashBucket), ht->numBuckets));

    tbNullChk (ht->iterators    = calloc(sizeof(unsigned __int64), kInitIteratorCapacity));
    ht->iteratorsCapacity       = kInitIteratorCapacity;

    *table = ht;
Error:
    if (error < 0 && ht)
    {
        HashTableDeallocate(ht);
        ht = NULL;
    }
...
int CVIFUNC HashTableInsertItem(HashTableType table, const void *key, const void *value)
{
    ...
    size_t              keySize;
    KeyHash             hash;
    KeyValuePairNodePtr newNode         = NULL;
    
    VALIDATE_TABLE  (table);
    VALIDATE_KEY    (key);
    VALIDATE_VALUE  (value);
    
    HASH_KEY (table, key, keySize, hash);

    // allocate node
    tbNullChk (newNode = calloc(sizeof(KeyValuePairNode), 1));
    tbNullChk (newNode->key = malloc(keySize));
    tbNullChk (newNode->value = malloc(table->valueSize));
    memcpy(newNode->key, key, keySize);
    memcpy(newNode->value, value, table->valueSize);
    
    errChk (HashTableInsertNodeInTable (table, &newNode, hash));
    insertedNode = 1;
    
    INVALIDATE_ALL_ITERATORS (table);
    
    // resize if necessary
    if (table->growLoad != VAL_DO_NOT_GROW
        && ((double)table->numOccupiedBuckets / (double)table->numBuckets) >= table->growLoad)
    {
        errChk (HashTableGrow(table));
    }
...
Error:
    if (error < 0)
    {
        ...
        {
            if (insertedNode)
                HashTableGetItemInternal(table, key, NULL, TRUE); // remove the item
            if (newNode->key)
                free(newNode->key);
            if (newNode->value)
                free(newNode->value);
...
int CVIFUNC HashTableGetItem(HashTableType table, const void *key, void *value, unsigned int valueBufferSize)
{
    int                 error   = 0;
    KeyValuePairNodePtr node    = NULL;
    
    VALIDATE_TABLE  (table);
    VALIDATE_KEY    (key);

    if (valueBufferSize == 0)
        return table->valueSize;

    VALIDATE_VALUE  (value);
    
    if (valueBufferSize < table->valueSize)
    {
        errChk (ToolErr_BufferTooSmall);
    }

    errChk (HashTableGetItemInternal(table, key, &node, FALSE));
    
    if (node)
        memcpy(value, node->value, table->valueSize);
    else
        memset(value, 0, valueBufferSize);
...
int CVIFUNC HashTableRemoveItem(HashTableType table, const void *key, void *value, unsigned int valueBufferSize)
{
    int                 error   = 0;
    KeyValuePairNodePtr node    = NULL;
    ...
    if (value && valueBufferSize < table->valueSize)
    {
        errChk (ToolErr_BufferTooSmall);
    }

    errChk (HashTableGetItemInternal(table, key, &node, TRUE));
    ...
    if (node)
    {
        ...
        // resize if necessary
        if (table->shrinkLoad != VAL_DO_NOT_SHRINK
            && ((double)table->numOccupiedBuckets / (double)table->numBuckets) <= table->shrinkLoad)
        {
            errChk (HashTableShrink(table));
        }
    }
    else if (value)
    {
...
int CVIFUNC HashTableClear(HashTableType table)
{
    ...
    {
        node = table->buckets[i].first;
        while (node)
        {
            restOfChain = node->next;
            free(node->key);
            free(node->value);
            free(node);
            node = restOfChain;
    ...
    memset(table->buckets, 0, numBuckets * sizeof(*table->buckets));
    table->size                 = 0;
    table->numOccupiedBuckets   = 0;
...
int CVIFUNC HashTableIteratorCreate(HashTableType table, HashTableIterator *iterator)
{
    ...
    *iterator = (HashTableIterator)0;
    
    // Find an unused iterator
    for (i = 1, n = table->iteratorsCapacity; i < n; ++i)
    {
        if (!ITERATOR_IS_RESERVED (table, i))
        {
            *iterator = (HashTableIterator)i;
            break;
        }
    }
    
    // Grow the iterator array if an empty iterator slot was not found
    if (*iterator == 0)
    {
        unsigned long       oldCapacity     = table->iteratorsCapacity,
                            newCapacity     = (oldCapacity == 0) ? kInitIteratorCapacity : 2 * oldCapacity;
        unsigned __int64    *newIterators;
        
        if (newCapacity > kMaxSupportedIterators)
            newCapacity = kMaxSupportedIterators;
        if (newCapacity <= oldCapacity)
            errChk (ToolErr_CannotAllocateIterator);
        
        tbNullChk (newIterators = calloc(sizeof(unsigned __int64), newCapacity)); /* assumes that 0 is invalid */
        memcpy(newIterators, table->iterators, oldCapacity * sizeof(unsigned __int64));
        free(table->iterators);
        table->iterators            = newIterators;
        table->iteratorsCapacity    = newCapacity;
        *iterator                   = (HashTableIterator)oldCapacity; /* first new iterator */
    }
    
    if (table->size == 0)
    {
        RESERVE_ITERATOR (table, *iterator);
        SET_ITERATOR_AT_END (table, *iterator);
        error = HASH_TABLE_END;
    }
    else
    {
        // Find the first pair in the table
        for (bucket = 0, n = table->numBuckets; bucket < n; ++bucket)
        {
            if (table->buckets[bucket].first)
                break;
        }
        SET_ITERATOR (table, *iterator, bucket, 0);
    }
...
int CVIFUNC HashTableIteratorGetItem(HashTableType table, HashTableIterator iterator, void *key, unsigned int keyBufferSize, void *value, unsigned int valueBufferSize)
{
    int                 error = 0;
    unsigned long       i, 
                        bucket,
    ...
    VALIDATE_TABLE                  (table);
    VALIDATE_ITERATOR_HAS_GOOD_ITEM (table, iterator);
    
    READ_ITERATOR (table, iterator, bucket, link);
    
    for (node = table->buckets[bucket].first, i = 0; i < link; ++i)
        node = node->next;

    keySize = (table->keyType == FIXED_SIZE_KEY) ? table->keySize : strlen(node->key) + 1;
    ...
    if (keyBufferSize < keySize)
        errChk (ToolErr_BufferTooSmall);
    if (valueBufferSize < table->valueSize)
        errChk (ToolErr_BufferTooSmall);

    memcpy(key,   node->key,   keySize);
    memcpy(value, node->value, table->valueSize);
...
int CVIFUNC HashTableIteratorAdvance(HashTableType table, HashTableIterator iterator)
{
    ...
    for (node = table->buckets[bucket].first, i = 0; i < link && node; ++i)
        node = node->next;

    // Set the iterator to the next pair
    if (node && node->next)
    {
        SET_ITERATOR (table, iterator, bucket, link + 1);
    }
    else
    {
        for (i = bucket + 1, n = table->numBuckets; i < n; ++i)
        {
            if (table->buckets[i].first)
            {
                SET_ITERATOR (table, iterator, i, 0);
                break;
            }
        }
        if (i == n)
...
int CVIFUNC HashTableIteratorDispose(HashTableType table, HashTableIterator iterator)
{
    ...
    VALIDATE_TABLE              (table);
    VALIDATE_ITERATOR_RESERVED  (table, iterator);

    FREE_ITERATOR (table, iterator);
Error:
    return error;
}
...
int CVIFUNC HashTableGetAttribute(HashTableType table, int attribute, void *value)
{
    ...
    switch (attribute)
    {
        case ATTR_HT_SIZE:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(unsigned long *)value = table->size;
            break;
        case ATTR_HT_NUM_BUCKETS:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(unsigned long *)value = table->numBuckets;
            break;
        case ATTR_HT_LOAD:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(double *)value        = (double)table->numOccupiedBuckets / (double)table->numBuckets;
            break;
        case ATTR_HT_GROW_LOAD:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(double *)value        = table->growLoad;
            break;
        case ATTR_HT_SHRINK_LOAD:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(double *)value        = table->shrinkLoad;
            break;
        case ATTR_HT_HASH_FUNCTION:
            if (!value)
                errChk (ToolErr_NullArgument);
            *(HashBytesFunc *)value = table->hashFunc;
            break;
        case ATTR_HT_HASH_DISTRIBUTION:
            if (!value)
                errChk (ToolErr_NullArgument);
            for (i = 0, n = table->numBuckets; i < n; ++i)
            {
                ((unsigned long *)value)[i] = table->buckets[i].chainLength;
            }
            break;
        default:
            errChk (ToolErr_InvalidAttribute);
    }
...
int CVIFUNC_C HashTableSetAttribute(HashTableType table, int attribute, ...)
{
    ...
    {
        case ATTR_HT_GROW_LOAD:
            load = va_arg(parmInfo, double);
            if (load != VAL_DO_NOT_GROW && (load < kMinGrowLoad || load > 1.0))
            {
                errChk (ToolErr_InvalidAttributeValue);
            }
            table->growLoad = load;
            // resize if necessary
            if (table->growLoad != VAL_DO_NOT_GROW
        ...
        case ATTR_HT_SHRINK_LOAD:
            load = va_arg(parmInfo, double);
            if (load != VAL_DO_NOT_SHRINK && (load < 0.0 || load > kMaxShrinkLoad))
            {
                errChk (ToolErr_InvalidAttributeValue);
            }
            table->shrinkLoad = load;
            // resize if necessary
            if (table->shrinkLoad != VAL_DO_NOT_SHRINK
                && ((double)table->numOccupiedBuckets / (double)table->numBuckets) <= table->shrinkLoad)
            {
            ...
            break;
        case ATTR_HT_HASH_FUNCTION:
            func = va_arg(parmInfo, HashBytesFunc);
            if (func == NULL)
                errChk (ToolErr_InvalidAttributeValue);
            table->hashFunc = func;
            // rehash all the resident pairs
            errChk (HashTableResize(table, table->numBuckets));
            break;
        case ATTR_HT_SIZE:              // not settable
        case ATTR_HT_NUM_BUCKETS:       // not settable
        case ATTR_HT_LOAD:              // not settable
        case ATTR_HT_HASH_DISTRIBUTION: // not settable
        default:
            errChk (ToolErr_InvalidAttribute);
    }
...
static int HashTableGrow(HashTableType table)
{
    return HashTableResize(table, 2 * table->numBuckets);
}
...
static int HashTableShrink(HashTableType table)
{
    unsigned long newSize = table->numBuckets / 2;

    return (newSize >= table->initialNumBuckets) ? HashTableResize(table, newSize) : 0;
}
...
static int HashTableResize(HashTableType table, unsigned long newNumBuckets)
{
    ...
    KeyValuePairNodePtr node,
                        restOfChain;
    unsigned long       oldNumBuckets   = table->numBuckets,
                        i;
    
    if (newNumBuckets > kMaxNumBuckets)
        errChk (ToolErr_FailedToResizeTable);
    
    tbNullChk (newBuckets = calloc(sizeof(HashBucket), newNumBuckets));
        
    table->buckets              = newBuckets;
    table->numBuckets           = newNumBuckets;
    table->size                 = 0;
    table->numOccupiedBuckets   = 0;
    ...
    {
        node = oldBuckets[i].first;
        while (node)
        {
            restOfChain = node->next;
            node->next = NULL;
            HASH_KEY (table, node->key, keySize, hash);
            if (HashTableInsertNodeInTable (table, &node, hash) < 0)
                errChk (ToolErr_FailedToResizeTable);
            node = restOfChain;
        }
    }
...
{
    int                 error       = 0;
    KeyValuePairNodePtr tempNode    = table->buckets[hash].first;
    KeyValuePairNodePtr node        = *pNode;
    
    while (tempNode)
    {
        // Replace the value if the key already exists
        if (table->keyCmpFunc(tempNode->key, node->key, table->keySize) == 0)
        {
            free(tempNode->value);
    ...
    // If not a replacement of an existing key-value binding, add the new binding to the front of the chain
    if (node)
    {
        if (table->buckets[hash].chainLength == kMaxChainLength)
        {
            errChk (ToolErr_ExceedsMaximumChainLength);
        }
        else
        {
            node->next = table->buckets[hash].first;
            table->buckets[hash].first = node;
            ++table->size;
            ++table->buckets[hash].chainLength;
            if (!node->next)
                ++table->numOccupiedBuckets;
        }
    }
Error:
    return error;
...
static int HashTableGetItemInternal(HashTableType table, const void *key, KeyValuePairNodePtr *foundNode, int remove)
{
    int                 error       = 0;
    size_t              keySize;
    KeyHash             hash;
    ...
    // replicate the for loop to avoid repeating the key type check
    for (node = table->buckets[hash].first; node; node = node->next)
    {
        if ((table->keyCmpFunc)(node->key, key, keySize) == 0)
        {
            if (remove)
            {
                --table->size;
                ...
                {
                    table->buckets[hash].first = node->next;
                    if (!node->next)
                        --table->numOccupiedBuckets;
                }
                node->next = NULL;
...
void HashTablePrint(HashTableType table)
{
    int error = 0;
    int i, n, count;
    ...
    printf( "size: %d, buckets: %d, load: %.1f%%, grow at %.1f%%, shrink at %.1f%%\n",
            table->size,
            table->numBuckets,
            100.0 * (((double)table->numOccupiedBuckets) / ((double)table->numBuckets)),
            (table->growLoad == VAL_DO_NOT_GROW) ? 0 : 100.0 * table->growLoad,
            (table->shrinkLoad == VAL_DO_NOT_SHRINK) ? 0 : 100.0 * table->shrinkLoad);

    printf("Item\tBucket\tKey\t\tValue\n-----------------------------------------------------------\n");
    for (i = 0, n = table->numBuckets, count = 0; i < n; ++i)
    {
        KeyValuePairNodePtr node;
        unsigned char *pByte;
                
        for (node = table->buckets[i].first; node; node = node->next)
        {
            printf("%d\t%d\t", count++, i);
            if (table->keyType == C_STRING_KEY)
            {
                printf(node->key);
            }
            else
            {
                for (pByte = node->key; pByte < (unsigned char *)node->key + table->keySize; ++pByte)
                    printf("%x", *pByte);
            }
            printf("\t");
            for (pByte = node->value; pByte < (unsigned char *)node->value + table->valueSize; ++pByte)
                    printf("%x", *pByte);
            printf("\n");
        }
    }
...
void CVIFUNC LocalizeNumberString (char *str)
{
    ...
    if (decimalSymbol != '.')
        {
        for (; *str; CmbStrInc((unsigned char *)str))
            if (*str == '.')
                *str = decimalSymbol;
        }
}
...
static int SharedTipPanel_ShowTip(SharedTipPanel *tipPanel, CtrlToolTip *tip)
{
    ...
    errChk( GetMonitorAttribute(monitorID, ATTR_LEFT, &screenRect.left));
...
{
#if defined(_NI_linux_) && _NI_linux_
    return SaveBitmapToBMPFile (BmpId, PathName);
#else
...
#if defined(_NI_mswin16_) && _NI_mswin16_
#define MAX_FILE_EXTENSION_LENGTH   3
#elif defined(_NI_mswin32_) && _NI_mswin32_
...
static int LocaleToLangId (char *locale)
{
    struct { char *locale; int length; int id; } table[] = {
        { "english",  7, LANG_ENGLISH  },   { "en_",      3, LANG_ENGLISH  },
        { "deutsch",  7, LANG_GERMAN   },   { "de_",      3, LANG_GERMAN   },
        { "francais", 8, LANG_FRENCH   },   { "fr_",      3, LANG_FRENCH   },
        { "japanese", 8, LANG_JAPANESE },   { "ja_",      3, LANG_JAPANESE },
        { NULL,       0, LANG_ENGLISH  }
    };
    int i;

    for (i = 0; table[i].length > 0; ++i)
        if (strncmp (locale, table[i].locale, table[i].length) == 0)
            return table[i].id;
    return LANG_ENGLISH;
}
#else
...
#if defined(_NI_linux_) && _NI_linux_
        strcpy (cvirtePathBuf, "/usr/local");
#else
        HANDLE  hCVIRTE;
...
#endif
            {
            char    *pc = (char *)_mbsrchr ((const unsigned char *)cvirtePathBuf, kFolderSeparatorChar);

            if (pc && NULL != (gCVIErrorList = ListCreate (sizeof (CVINiErrInfo))))
                {
#if defined(_NI_linux_) && _NI_linux_
                int primaryLangId = LocaleToLangId (setlocale (LC_MESSAGES, NULL));
#else
...
char * CVIFUNC GetToolboxErrorString(int errorCode)
{
    ...
    switch (-absError)
        ...
        case ToolErr_InvalidHashTable:
            return "The hash table is invalid.";
        case ToolErr_InvalidKeyType:
            return "The key type is invalid.";
        case ToolErr_InvalidKeySize:
            return "The key size is invalid.";
        case ToolErr_InvalidValueSize:
            return "The value size is invalid.";
        case ToolErr_InvalidKeyArgument:
            return "The key argument is invalid.";
        case ToolErr_InvalidValueArgument:
            return "The value argument is invalid.";
        case ToolErr_NullArgument:
            return "A null pointer was passed when a non-null pointer was expected.";
        case ToolErr_BufferTooSmall:
            return "The buffer is not large enough to hold the data.";
        case ToolErr_InvalidAttribute:
            return "The attribute passed is invalid.";
        case ToolErr_InvalidAttributeValue:
            return "The attribute value passed is invalid.";
        case ToolErr_InvalidIterator:
            return "The iterator is no longer valid because the hash table has had items added or removed.";
        case ToolErr_UnallocatedIterator:
            return "The iterator is not allocated for use.";
        case ToolErr_CannotAllocateIterator:
            return "Cannot allocate iterator. Your program must dispose hash table iterators when it is done using them.";
        case ToolErr_IteratorAtEnd:
            return "The iterator references the end of the hash table.";
        case ToolErr_ExceedsMaximumTableSize:
            return "Operation would exceed the maximum number of hash table buckets.";
        case ToolErr_ExceedsMaximumChainLength:
            return "Operation would exceed the maximum hash bucket chain length.";
        case ToolErr_FailedToResizeTable:
            return "Failed to resize the hash table.";
...
char * CVIFUNC GetGeneralErrorString(int errorCode)
{
    ...
    static const char   *kErrorsDirRegLoc = "/usr/local/natinst/share/errors/";
#else
    static const char   *kEnglishFile = "English\\*.txt";
    static const char   *kJapaneseFile = "Japanese\\*.txt";
    static const char   *kFrenchFile = "French\\*.txt";
    ...
    if (gSharedNIErrorList == NULL)
...
#if defined(_NI_linux_) && _NI_linux_
        strcpy (errorFilesPathBuf, kErrorsDirRegLoc);
#else
        if (0 == RegReadString (REGKEY_HKLM, kErrorsDirRegLoc, kErrorsDirValueName,
         (unsigned char *)errorFilesPathBuf, sizeof (errorFilesPathBuf), &size))
            ...
            if (*_mbsdec ((const unsigned char *)errorFilesPathBuf, (unsigned char *)pc) != kFolderSeparatorChar)
                {
                errorFilesPathBuf[0] = kFolderSeparatorChar;
                errorFilesPathBuf[1] = '\0';
                }

#if defined(_NI_linux_) && _NI_linux_
            switch (LocaleToLangId (setlocale (LC_MESSAGES, NULL)))
#else
            ...
            pc = (char *)_mbsrchr ((const unsigned char *)errorFilesPathBuf, kFolderSeparatorChar);
            pc = (char *)_mbsinc ((unsigned char *)pc);
...
#endif /* (defined(_NI_mswin16_) && _NI_mswin16_) || (defined(_NI_mswin32_) && _NI_mswin32_) */
...
#if HAVE_CVI_RTE
int CVIFUNC GetMetaFontInfoEx(const char metaFont[], char typeface[], int *bold, int *underline, int *strikeout,
    int *italic, int *size, int *angle)
{
    int             error = UIENoError;
    ThreadLocalVars *pThreadLocalVars = 0;
    ...
    errChk(GetMetaFontInfo(metaFont, typeface, bold, underline, strikeout, italic, size));

    if (angle)
        errChk( GetCtrlAttribute(pThreadLocalVars->fontPanel, pThreadLocalVars->txtMsg, ATTR_TEXT_ANGLE, angle));
...
int CVIFUNC AcceleratorEscapeCodeLocation(const char *text)
{
    ...
    if( text)
        ...
        for (;length > 2; CmbStrInc((unsigned char *)text), length--)
            if (*text == '_' && *(text+1) == '_')
...
char * CVIFUNC UILEventString(int event)
{
    ...
    switch (event)
        ...
        case EVENT_LEFT_CLICK:                  return "EVENT_LEFT_CLICK";
        case EVENT_LEFT_DOUBLE_CLICK:           return "EVENT_LEFT_DOUBLE_CLICK";
        case EVENT_RIGHT_CLICK:                 return "EVENT_RIGHT_CLICK";
        case EVENT_RIGHT_DOUBLE_CLICK:          return "EVENT_RIGHT_DOUBLE_CLICK";
        case EVENT_KEYPRESS:                    return "EVENT_KEYPRESS";
        case EVENT_GOT_FOCUS:                   return "EVENT_GOT_FOCUS";
        case EVENT_LOST_FOCUS:                  return "EVENT_LOST_FOCUS";
        ...
        case EVENT_ACTIVE_CELL_CHANGE:          return "EVENT_ACTIVE_CELL_CHANGE";
        case EVENT_ROW_SIZE_CHANGE:             return "EVENT_ROW_SIZE_CHANGE";
        case EVENT_COLUMN_SIZE_CHANGE:          return "EVENT_COLUMN_SIZE_CHANGE";
        case EVENT_ACTIVE_TAB_CHANGE:           return "EVENT_ACTIVE_TAB_CHANGE";
        ...
        case EVENT_MOUSE_WHEEL_SCROLL:          return "EVENT_MOUSE_WHEEL_SCROLL";
        ...
        case EVENT_TABLE_ROW_COL_LABEL_CLICK:   return "EVENT_TABLE_ROW_COL_LABEL_CLICK";
        default:
    ...
    if (!(posInList = FindMsgCBHandlerFromHPanel (hpanel, &phandler)))
        ...
        switch (attribute)
            ...
            case ATTR_ENABLED:
                ...
                break;
            case ATTR_CALL_DEF_WIN_PROC:
                plink->callDefProc = va_arg(parg, int);
                break;
            case ATTR_WIN_PROC_RET_VAL:
                plink->winProcRetVal = va_arg(parg, int);
                break;
            default:
        ...
        switch (attribute)
            ...
            case ATTR_ENABLED:
                *(int*)pval = plink->enabled;
                break;
            case ATTR_CALL_DEF_WIN_PROC:
                *(int*)pval = plink->callDefProc;
                break;
            case ATTR_WIN_PROC_RET_VAL:
                *(int*)pval = plink->winProcRetVal;
                break;
            default:
                retVal = UIEInvalidAttribute;
            }
            ...
            for (linkPosInList = 1; linkPosInList <= numLinks; linkPosInList++)
                ...
                if (plink->enabled)
                    ...
                    if (message == (UINT)(plink->msgNum))
                        {
                        callDefProc = plink->callDefProc;
                        winProcRetVal = plink->winProcRetVal;
                                          ...
                                          void* pcallbackData)
{
    ...
    newLink.callDefProc = TRUE;
    newLink.winProcRetVal = 0;
...
#if (defined(_NI_mswin32_) && _NI_mswin32_) || (defined(_NI_linux_) && _NI_linux_)
//-------------------------------------------------------------------------------------------------
...
#endif /* (defined(_NI_mswin32_) && _NI_mswin32_) || (defined(_NI_linux_) && _NI_linux_) */
